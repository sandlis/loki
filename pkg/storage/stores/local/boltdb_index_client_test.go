...
func queryTestBoltdb(t *testing.T, boltdbIndexClient *BoltdbIndexClientWithArchiver, query chunk.IndexQuery) []string {
	resp := []string{}
...
func buildTestBoltDBIndexClient(boltdbDir, archiveCacheLocation, ingesterName, archiverLocalStoreLocation string) (*BoltdbIndexClientWithArchiver, error) {
	...
	if archiveCacheLocation != "" {
		...
		archiveStoreClient, err := local.NewFSObjectClient(archiveConfig.StoreConfig.FSConfig)
		if err != nil {
			return nil, err
		}
	...
	return indexClient.(*BoltdbIndexClientWithArchiver), nil
}
